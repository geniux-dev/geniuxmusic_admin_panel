...
$fetch_tickets_years = mysqli_query($conn,"SELECT * FROM reports_tickets");
$_SESSION['year']='';
...
<div class="menuu" id="selectYears" style="display: none;">
    ...
    <li onclick="filter_posts('<?php echo date('Y', time());?>')" class="bordB"><a href="javascript:void(0);"><?php echo date('Y', time());?></a></li>    
        <?php
            while($fetch_tickets_years_r = mysqli_fetch_assoc($fetch_tickets_years)){
                $years = substr($fetch_tickets_years_r['date_created'], 0, 4);
...
<script>
            ...
            <?php
                for($i=1;$i<=12;$i++){
                    ...
                    if($yearmonth<=date('Y-m', time())){
                        ...
                        $uploadedposts = mysqli_query($conn,"SELECT * FROM reports_tickets WHERE date_created LIKE '%$yearmonth%'");
        ...
        $('#graphTitle').html('Tickets Created (<?php echo $year;?>)');
</script>       
<script>
